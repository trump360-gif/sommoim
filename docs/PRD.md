...
| 관심사 설정 | 카테고리별 관심사 선택 | P1 | ✅ 완료 |
| 팔로우/팔로잉 | 관심 사용자 팔로우/언팔로우 | P1 | ✅ 완료 |
| 팔로워/팔로잉 목록 | 팔로워/팔로잉 사용자 조회 | P1 | ✅ 완료 |
| 얼굴 사진 | 선택, 모임주가 참가자 확인용 | P2 | 🔜 예정 |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 운영진 추가 | 참가자를 운영진으로 지정 | ✅ 완료 |
| 역할 설정 | CO_HOST, MANAGER, STAFF | ✅ 완료 |
| 권한 설정 | 일정/활동/회원/채팅/통계 관리 권한 | ✅ 완료 |
| 운영진 해제 | 운영진 권한 제거 | ✅ 완료 |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 관심사 기반 추천 | 사용자 관심사와 일치하는 모임 우선 추천 | ✅ 완료 |
| 인기 모임 | 조회수 기반 인기 모임 | ✅ 완료 |
| 추천 이유 표시 | 추천 근거 태그 표시 | ✅ 완료 |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 질문 관리 | 모임주가 가입 질문 추가/수정/삭제 | ✅ 완료 |
| 필수 여부 | 질문별 필수/선택 설정 | ✅ 완료 |
| 답변 제출 | 가입 신청 시 질문 답변 | ✅ 완료 |
| 답변 조회 | 모임주가 신청자 답변 확인 | ✅ 완료 |
| 일괄 승인/거절 | 여러 신청 동시 처리 | ✅ 완료 |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 북마크 추가 | 모임 상세 페이지에서 북마크 | ✅ 완료 |
| 북마크 해제 | 북마크된 모임 해제 | ✅ 완료 |
| 북마크 목록 | 내가 북마크한 모임 목록 조회 | ✅ 완료 |
| 북마크 페이지 | /bookmarks 페이지에서 관리 | ✅ 완료 |
...
| 지표 | 설명 | 상태 |
|------|------|------|
| 통계 카드 | 사용자/모임/활동/신고 통계 (아이콘, 색상, 트렌드) | ✅ 완료 |
| 빠른 액션 | 콘텐츠 관리, 홈페이지 관리 바로가기 | ✅ 완료 |
| 최근 신고 | 최근 신고 목록 실시간 표시 | ✅ 완료 |
| 상대적 시간 | 방금 전, n분 전 등 시간 표시 | ✅ 완료 |
...
|------|------|------|
| 사용자 목록 | 검색, 페이지네이션 | ✅ 완료 |
| 사용자 정렬 | 닉네임/이메일/권한/가입일 기준 정렬 | ✅ 완료 |
...
├── /meetings               - 모임 목록 ✅
│   ├── /create             - 모임 생성 ✅
│   └── /[id]               - 모임 상세 (탭: 정보/활동/리뷰/운영진/설정) ✅
│       ├── /edit           - 모임 수정 ✅
│       └── /chat           - 모임 채팅 ✅
│
├── /mypage                 - 마이페이지 ✅
│   ├── /edit               - 프로필 수정 (관심사 설정 포함) ✅
│   ├── /meetings           - 내 모임 (만든 모임/참여 모임) ✅
│   ├── /calendar           - 내 일정 (캘린더 뷰) ✅
...
- 탭 네비게이션: 모임 정보 / 활동 기록 / 리뷰 / 운영진(호스트) / 설정(호스트)
- 모임 이미지 및 기본 정보
- 일정 목록
- 참가자 목록 (운영진 배지 표시)
- 참가 신청/채팅 버튼
...
| - | 관리자 대시보드 UI 개선 (통계 카드, 빠른 액션) | ✅ 완료 | 2025-12-06 |
| - | 사용자 관리 정렬/필터 기능 | ✅ 완료 | 2025-12-06 |
| - | 관리자 사이드바 Lucide 아이콘 적용 | ✅ 완료 | 2025-12-06 |
| - | 토스트 알림 (sonner) 도입 | ✅ 완료 | 2025-12-06 |
| - | 드래그앤드롭 정렬 (dnd-kit) 도입 | ✅ 완료 | 2025-12-06 |
| 7 | 운영진 권한 관리 (역할/권한 설정) | ✅ 완료 | 2025-12-09 |
| 7 | 가입 질문/심사 기능 | ✅ 완료 | 2025-12-09 |
...
| - | 메인 페이지 섹션 순서 동적 반영 | ✅ 완료 | 2025-12-10 |
| - | 관리자 사이드바 텍스트 한글화 (ADMIN → 관리자) | ✅ 완료 | 2025-12-10 |
| - | 배너 이미지 제거 시 null 처리 | ✅ 완료 | 2025-12-10 |
| - | 이미지 업로더 URL 직접 입력 기능 | ✅ 완료 | 2025-12-10 |
| 9 | 메인 페이지 5대 개선 (애니메이션, 모바일 네비) | ✅ 완료 | 2025-12-13 |
| 9 | 마이페이지 기능 확장 (내 모임, 캘린더, 알림 설정) | ✅ 완료 | 2025-12-13 |
| 9 | 사이드바 섹션 동적 네비게이션 | ✅ 완료 | 2025-12-13 |
| 9 | 코드 리팩토링 (공유 모듈, 컴포넌트 분리) | ✅ 완료 | 2025-12-13 |
...
POST   /api/meetings/:id/apply                  ✅ 가입 신청 (답변 포함)
GET    /api/meetings/:id/applications           ✅ 신청 목록
PUT    /api/meetings/applications/:id/review    ✅ 신청 심사
POST   /api/meetings/:id/applications/bulk-review ✅ 일괄 심사
```
